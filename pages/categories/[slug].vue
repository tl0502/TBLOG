...
import { fetchCategoryDetail, useCategoryDetail } from '~/composables/usePublicApi'
...
const { data, error } = await useCategoryDetail(slug)
...
if (error.value && import.meta.server) {
  ...
  if (event) setResponseStatus(event, 503, 'Category temporarily unavailable')
}
if (error.value && import.meta.client && !useNuxtApp().isHydrating) {
  throw createError({ statusCode: 503, statusMessage: 'Category temporarily unavailable', fatal: true })
}
...
async function loadMore() {
  ...
  loadError.value = ''
  try {
    const response = await fetchCategoryDetail(slugAtStart, { cursor: cursorAtStart })
    if (
      slug.value !== slugAtStart
      || response.data.category.slug !== slugAtStart
...
watch([slug, data], ([currentSlug, response]) => {
  ...
  if (response?.data?.category.slug !== currentSlug) {
    if (identityChanged) {
      articleIds.clear()
      articles.value = []
...
useBasicPageSeo({
  title: () => detail.value?.category.name,
  description: () => detail.value?.category.description
})
</script>
