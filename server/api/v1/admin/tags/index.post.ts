...
import { createTagInputSchema } from '../../../../validation/admin-taxonomy-input'

export default defineEventHandler(async (event) => {
  try {
    await requireAdmin(event)
    const input = createTagInputSchema.parse(await readBody(event))
    const tag = await createAdminTaxonomyServiceForEvent(event).createTag(input)
