...
export default defineEventHandler(async (event) => {
  try {
    ...
    const detail = await createTaxonomyReadServiceForEvent(event).getTagDetail(slug, query)
    setPublicNoStoreHeaders(event)
    return ok({ tag: detail.tag, items: detail.articles.items }, { nextCursor: detail.articles.nextCursor })
  } catch (error) {
    const mapped =
